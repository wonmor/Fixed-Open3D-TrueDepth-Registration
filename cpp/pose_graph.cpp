...
void cpp_optimize_pose_graph_with_matches(std::vector<double> &poses, const std::vector<double> &matches) {
    ...
    for (size_t i=0; i < matches.size(); i+=MATCH_DIM) {
        ...
        problem.AddResidualBlock(cost,
            ...
            // Use SetManifold instead of SetParameterization
            problem.SetManifold(poses.data() + id1 * POSE_DIM, quaternion_manifold);
            problem.SetManifold(poses.data() + id2 * POSE_DIM, quaternion_manifold);
    }
